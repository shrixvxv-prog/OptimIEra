...
The command requires `NODE_ENV=production`, a non-local managed `DATABASE_URL`, an existing owner account, and public 0G Storage/Chain read configuration. It never uploads evidence, sends a chain transaction, or creates a new certificate on 0G.
