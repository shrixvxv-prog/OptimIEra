...
- The Rules Engine remains the default provider and is deterministic/local. Nous prompt intelligence is configured in the current local environment, but external provider execution remains opt-in and is disabled in safe public mode.
- The 0G Compute Router is configured on Galileo testnet and the current preflight/live check reached the Router successfully. Deployment environments still need their own server-side key and explicit live-write policy.
...
- Public live 0G writes are disabled by default. Enabling them requires explicit testnet configuration and is bounded by database-backed per-user and global daily quotas.

- Phase 7 live activation is testnet-ready in the current local environment: Compute inference succeeded, Storage/Chain read checks succeeded, and the registry has deployed bytecode. Uploads, transactions, and public live writes remain explicitly skipped unless separately enabled.
